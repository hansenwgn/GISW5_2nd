...
# cek kalau data bukan spatial, cek st_crs(a)
# WGS 84 / 4326 geodetic coordinate system (unitnya degree minute second)
# 27700 London projected coordinate system (unitnya meter)
...
Hotels <- Joinfun(Londonborough, OSM)

```

```{r}
# then for airbnb
Airbnb3 <- Joinfun(Londonborough, Airbnb2)
```

```{r}
#select chosen cities in UK
...
legend <- tm_shape(Hotels_groupby) +
    tm_polygons("Accomodation count",
                palette="PuBu") +
...
```{r}
# Inset Map
Londonbb <- st_bbox(Airbnb_groupby,
                    crs = st_crs(Airbnb))%>%
...
```{r}
main <- tm_shape(Airbnb_groupby, bbbox = Londonbb) + 
  tm_polygons("Accomodation count",
              breaks=breaks, 
              palette="PuBu")+
...
```{r}
#join data
Joined <- Airbnb_groupby%>%
  st_join(., Hotels_groupby, join = st_equals)%>%
